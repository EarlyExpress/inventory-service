...
class InventoryServiceTest {
    ...
    class CreateInitialInventoriesTest {
        ...
        void createInitialInventories_Success() {
            ...
            verify(eventProducer, times(4)).publishInventoryCreated(any(Inventory.class));
        }
        ...
        void createInitialInventories_SkipExisting() {
            ...
            // then
            verify(inventoryRepository, times(3)).save(any(Inventory.class)); // 3개만 생성
            verify(eventProducer, times(3)).publishInventoryCreated(any(Inventory.class));
        }
    ...
    class RestockTest {
        ...
        void restock_Success() {
            ...
            // when
            Inventory result = inventoryService.restock(TEST_INVENTORY_ID, restockQuantity);

            // then
            assertThat(result.getQuantityInHub().getValue()).isEqualTo(expectedQuantity);
            assertThat(result.getLastRestockedAt()).isNotNull();
            verify(eventProducer).publishInventoryRestocked(any(Inventory.class), eq(restockQuantity));
        }
        ...
        void restock_InventoryNotFound() {
            ...
            // when & then
            assertThatThrownBy(() -> inventoryService.restock(TEST_INVENTORY_ID, 50))
    ...
    class ReserveStockTest {
        ...
        void reserveStock_Success() {
            ...
            // when
            Inventory result = inventoryService.reserveStock(TEST_INVENTORY_ID, reserveQuantity);

            // then
            assertThat(result.getReservedQuantity().getValue()).isEqualTo(reserveQuantity);
            assertThat(result.getAvailableQuantity().getValue()).isEqualTo(70); // 100 - 30
            verify(inventoryRepository).save(any(Inventory.class));
        }

        @Test
        @DisplayName("가용 재고 부족 시 예약 실패")
        void reserveStock_InsufficientStock() {
            // given
            int reserveQuantity = 150; // 가용 재고보다 많음
            given(inventoryRepository.findById(TEST_INVENTORY_ID))
                    .willReturn(Optional.of(testInventory));

            // when & then
            assertThatThrownBy(() -> inventoryService.reserveStock(TEST_INVENTORY_ID, reserveQuantity))
                    .isInstanceOf(InventoryException.class)
                    .hasFieldOrPropertyWithValue("errorCode", InventoryErrorCode.INSUFFICIENT_AVAILABLE_STOCK);
        }
        ...
        void reserveStock_PublishLowStockEvent() {
            ...
            // when
            inventoryService.reserveStock(TEST_INVENTORY_ID, reserveQuantity);

            // then
            verify(eventProducer).publishInventoryLowStock(any(Inventory.class));
        }
    }
    ...
    class ReleaseReservationTest {
        ...
        void releaseReservation_Success() {
            ...
            // when
            Inventory result = inventoryService.releaseReservation(TEST_INVENTORY_ID, releaseQuantity);
        ...
        void releaseReservation_ExceedReservedQuantity() {
            ...
            // when & then
            assertThatThrownBy(() -> inventoryService.releaseReservation(TEST_INVENTORY_ID, releaseQuantity))
                    .isInstanceOf(InventoryException.class)
    ...
    class ConfirmShipmentTest {
        ...
        void confirmShipment_Success() {
            ...
            // when
            Inventory result = inventoryService.confirmShipment(TEST_INVENTORY_ID, shipmentQuantity);
        ...
        void confirmShipment_PublishLowStockEvent() {
            ...
            // when
            inventoryService.confirmShipment(TEST_INVENTORY_ID, shipmentQuantity);
